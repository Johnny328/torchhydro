#!/usr/bin/env python3
"""
Updated spatial sensitivity test results comparison plotting script
Compare observed values with sim1/sim2/sim3 spatial scenario predictions

Updated paths:
- Observations: /home/lizilin/code/torchhydro/results/gnn_experiment/test_sim/epoch60flow_obs.nc
- Predictions: /home/lizilin/code/torchhydro/results/gnn_experiment/songliao_3h_test_{mode}/epoch60flow_pred.nc
- Output: /home/lizilin/code/torchhydro/results/gnn_experiment/test_sim/
...
def load_spatial_sensitivity_data(base_dir):
    """Load all spatial sensitivity test data with updated paths"""
    base_dir = Path(base_dir)
    
    # Define file paths with new structure
    files = {
        #'obs': base_dir / "test_sim" / "epoch60flow_obs.nc",
        'obs': base_dir / "songliao_3h_test_normal"  / "epoch60flow_obs.nc",
        'normal': base_dir / "songliao_3h_test_normal" / "epoch60flow_pred.nc",
        'sim1': base_dir / "songliao_3h_test_sim1" / "epoch60flow_pred.nc",
        'sim2': base_dir / "songliao_3h_test_sim2" / "epoch60flow_pred.nc", 
        'sim3': base_dir / "songliao_3h_test_sim3" / "epoch60flow_pred.nc"
    }
    ...
    if missing_files:
        ...
        for file in missing_files:
            print(f"  ✗ {file}")
        # Don't return None, continue with available files
...
def plot_spatial_comparison_period(obs_flow, sim_flows, period, basin_names, time_array, save_dir):
    ...
    # Plot normal mode (baseline) and three scenario predictions
    colors = ['#ff7f0e', '#1f77b4', '#2ca02c', '#d62728']  # orange, blue, green, red
    styles = ['-', '-', '--', '-.']
    labels = ['Normal (Baseline)', 'Sim1 (Upstream Heavy Rain)', 'Sim2 (Midstream Heavy Rain)', 'Sim3 (Downstream Heavy Rain)']
    
    metrics_list = []
    
    for i, (sim_name, (color, style, label)) in enumerate(zip(['normal', 'sim1', 'sim2', 'sim3'], 
                                                              zip(colors, styles, labels))):
        if sim_name in sim_flows and sim_flows[sim_name] is not None:
            print(f"  Debug - {sim_name} shape: {sim_flows[sim_name].shape}")
            sim_subset = sim_flows[sim_name][basin_idx, start_idx:end_idx]
            print(f"  Debug - {sim_name} subset shape: {sim_subset.shape}")
            print(f"  Debug - {sim_name} subset range: {np.min(sim_subset):.3f} - {np.max(sim_subset):.3f}")
            ...
            print(f"  Warning - {sim_name} data not available or None")
    ...
    if metrics_list:
        ...
        for metrics in metrics_list:
            scenario_name = {'normal': 'Normal', 'sim1': 'Sim1', 'sim2': 'Sim2', 'sim3': 'Sim3'}[metrics['scenario']]
...
def plot_spatial_sensitivity_comparison(data, save_dir):
    ...
    if obs_flow is None:
        ...
        print(f"Using first available variable: {first_var}")
    
    # 提取预测数据
    sim_flows = {}
    for sim_name in ['normal', 'sim1', 'sim2', 'sim3']:
        if sim_name in data and data[sim_name] is not None:
            sim_data = data[sim_name]
            print(f"\nProcessing {sim_name} data:")
            print(f"  Available variables: {list(sim_data.keys())}")
            
            # 尝试找到流量数据
            sim_flow = None
            for var_name in flow_var_names:
                if var_name in sim_data:
                    sim_flow = sim_data[var_name]
                    print(f"  Found {sim_name} flow data with variable name: {var_name}")
                    print(f"  {sim_name} flow shape: {sim_flow.shape}")
                    break
            
            if sim_flow is None:
                # 使用第一个数据变量
                if sim_data:
                    first_var = list(sim_data.keys())[0]
                    sim_flow = sim_data[first_var]
                    print(f"  Using first available variable for {sim_name}: {first_var}")
                    print(f"  {sim_name} flow shape: {sim_flow.shape}")
            
            # 检查数据维度是否匹配
            if sim_flow is not None:
                print(f"  {sim_name} data range: {np.min(sim_flow):.3f} - {np.max(sim_flow):.3f}")
                if sim_flow.shape != obs_flow.shape:
                    print(f"  WARNING: {sim_name} shape {sim_flow.shape} != obs shape {obs_flow.shape}")
                sim_flows[sim_name] = sim_flow
            else:
                print(f"  ERROR: No valid data found for {sim_name}")
                sim_flows[sim_name] = None
        else:
            print(f"  WARNING: {sim_name} data not loaded or None")
            sim_flows[sim_name] = None
...
"""
        for scenario in ['normal', 'sim1', 'sim2', 'sim3']:
            ...
            if len(scenario_data) > 0:
                ...
                scenario_names = {'normal': 'Baseline (Normal)', 'sim1': 'Upstream Heavy Rain', 'sim2': 'Midstream Heavy Rain', 'sim3': 'Downstream Heavy Rain'}
                summary_text += f"""
...
def main():
    """Main function"""
    # Set base directory for gnn_experiment results
    base_dir = Path("/home/lizilin/code/torchhydro/results/gnn_experiment")
    
    print("=== Updated Spatial Sensitivity Test Results Comparison Analysis ===")
    print(f"Base directory: {base_dir}")
    print("Data paths:")
    print(f"  Observations: {base_dir}/test_sim/epoch60flow_obs.nc")
    print(f"  Normal predictions: {base_dir}/songliao_3h_test_normal/epoch60flow_pred.nc")
    print(f"  Sim1 predictions: {base_dir}/songliao_3h_test_sim1/epoch60flow_pred.nc")
    print(f"  Sim2 predictions: {base_dir}/songliao_3h_test_sim2/epoch60flow_pred.nc")
    print(f"  Sim3 predictions: {base_dir}/songliao_3h_test_sim3/epoch60flow_pred.nc")
    ...
    # Load data
    print("\n1. Loading data...")
    data = load_spatial_sensitivity_data(base_dir)
    ...
    save_dir = base_dir / "test_sim"
    plot_spatial_sensitivity_comparison(data, save_dir)
